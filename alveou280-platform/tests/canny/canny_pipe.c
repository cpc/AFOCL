...
init(
    ...
    if (VERBOSE) {
        for (unsigned int r = 0; r < ret_num_devices; ++r) {
            ...
            if (strstr(bik_list, "pocl.sobel3x3.p512")) {
            ...
            if (strstr(bik_list, "pocl.magnitude.p512")) {
                magnitude_kernel_enabled = 1;
                magnitude_kernel_device_idx = r;
            }
            if (strstr(bik_list, "pocl.oriented.nonmaxsuppression.p512")) {
                suppr_kernel_enabled = 1;
                suppr_kernel_device_idx = r;
            }
            if (strstr(bik_list, "pocl.canny.p512")) {
            ...
            commandQueue[r] = clCreateCommandQueue(
                    context, deviceIds[r], CL_QUEUE_PROFILING_ENABLE | CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE, &retval);
            if (retval != CL_SUCCESS) {
    ...
    stream_in_kernel = clCreateKernel(program[sobel_kernel_device_idx], "pocl.stream.mm2s.p512", &retval);
    if (retval != CL_SUCCESS) {
        printf("Kernel creation error: %s\n", clErrorString(retval));
    }
    retval = clSetKernelArg(stream_in_kernel, 0, sizeof(cl_mem), &input_buffer);
    if (retval != CL_SUCCESS) {
    ...
    stream_out_kernel = clCreateKernel(program[sobel_kernel_device_idx], "pocl.stream.s2mm.p512", &retval);
    if (retval != CL_SUCCESS) {
        printf("Kernel creation error: %s\n", clErrorString(retval));
    }
    ...
    retval = clSetKernelArg(stream_out_kernel, 1, sizeof(cl_mem), &suppressed_buffer);
    if (retval != CL_SUCCESS) {
        printf("Stream out argument setting error: %s\n", clErrorString(retval));
    }
    bcast_x_kernel = clCreateKernel(program[sobel_kernel_device_idx], "pocl.broadcast.1to2.p512", &retval);
    if (retval != CL_SUCCESS) {
        printf("Kernel creation error: %s\n", clErrorString(retval));
    }
    ...
    bcast_y_kernel = clCreateKernel(program[sobel_kernel_device_idx], "pocl.broadcast.1to2.p512", &retval);
    if (retval != CL_SUCCESS) {
        printf("Kernel creation error: %s\n", clErrorString(retval));
    }
    ...
    if (sobel_kernel_enabled) {
        sobel_kernel = clCreateKernel(program[sobel_kernel_device_idx], "pocl.sobel3x3.p512", &retval);
        if (retval != CL_SUCCESS) {
            printf("Kernel creation error: %s\n", clErrorString(retval));
        }
    ...
    if (phase_kernel_enabled) {
        phase_kernel = clCreateKernel(program[phase_kernel_device_idx], "pocl.phase.p512", &retval);
        if (retval != CL_SUCCESS) {
            printf("Kernel creation error: %s\n", clErrorString(retval));
        }
        retval =
    ...
    if (magnitude_kernel_enabled) {
        magnitude_kernel = clCreateKernel(program[magnitude_kernel_device_idx], "pocl.magnitude.p512", &retval);
        if (retval != CL_SUCCESS) {
            printf("Kernel creation error: %s\n", clErrorString(retval));
        }
        retval =
    ...
    if (suppr_kernel_enabled) {
        nonmax_suppr_kernel = clCreateKernel(program[suppr_kernel_device_idx], "pocl.oriented.nonmaxsuppression.p512", &retval);
        if (retval != CL_SUCCESS) {
            printf("Kernel creation error: %s\n", clErrorString(retval));
        }
    ...
    if (canny_kernel_enabled) {
        canny_kernel = clCreateKernel(program[canny_kernel_device_idx], "pocl.canny.p512", &retval);
        if (retval != CL_SUCCESS) {
            printf("Kernel creation error: %s\n", clErrorString(retval));
        }
        retval = clSetKernelArg(
...
cannyEdgeDetection(
    ...
    if (sobel_kernel_enabled) {
        ...
        if (phase_kernel_enabled && magnitude_kernel_enabled) {
            clEnqueueNDRangeKernel(
                    commandQueue[phase_kernel_device_idx], bcast_x_kernel, 2, NULL, global, NULL, 0, NULL, NULL);
            clEnqueueNDRangeKernel(
                    commandQueue[phase_kernel_device_idx], bcast_y_kernel, 2, NULL, global, NULL, 0, NULL, NULL);
            clEnqueueNDRangeKernel(
            ...
            if (suppr_kernel_enabled) {
                ...
                clEnqueueNDRangeKernel(
                        commandQueue[suppr_kernel_device_idx], stream_out_kernel, 2, NULL, global, NULL, 0, NULL, &e5);
                clEnqueueReadBuffer(
